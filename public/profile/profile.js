...
function SwitchTab(tab) {
  ...
  activetab.tab.children(".main-chat-container").scrollTop(9999999999);
}
      ...
      relevantUsers.forEach((ru) => {
        if (ru.username == u.username) {
          if (ru.online != u.online) {
            u.update(ru.online);
            sendNotif(`User ${ru.username} ${ru.online ? "joined" : "left"}`);
          }
        }
      });
    });
...
class SettingsTab {
  constructor() {
    ...
    // this.tabcontainer = $("#settings-container");
    // this.selbar = tab.children(".settings-bar");
    // this.tabbuttons = this.selbar.children();
    // this.tabs = this.selbar.children();

    // this.activetab = this.tabs[0];
    // for (let i = 0; i < this.tabs.length; i++) {
    //   $(this.tabbuttons[i]).click(() => {});
    // }
  }
  updateUsers() {}
