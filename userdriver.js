...
dbWrapper
    ...
    .then(async (dBase) => {
        ...
        try {
            ...
            await db.run(
                "CREATE TABLE Users (username TEXT, hash TEXT, permission TEXT, friends TEXT)"
            );
        } catch (dbError) {}
    });
...
module.exports = {
    ...
    addUser: async (username, hash) => {
        ...
        try {
            success = await db.run(
                "INSERT INTO Users (username,hash,permission,friends) VALUES (?,?,?,?)",
    ...
    updateUser: async (username, permission, friends) => {
        let success = false;
        try {
            success = await db.run(
                "Update Users SET permission = ? WHERE username = ?",
                [permission, username, friends]
            );
        } catch (dbError) {
            console.error(dbError);
        }
