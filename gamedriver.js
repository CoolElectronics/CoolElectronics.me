  ...
  .then(async (dBase) => {
    ...
    try {
      await db.run(
        "CREATE TABLE " +
          dbname +
          " (category TEXT, name TEXT, description TEXT, embedurl TEXT)"
      );
    } catch (dbError) {}
  });
module.exports = {
  ...
  addGame: async (cat, name, desc, embedurl) => {
    ...
    try {
      success = await db.run(
        "INSERT INTO ? (category, name, description, embedurl) VALUES (?,?,?,?)",
  ...
  updateGame: async (oldname, cat, newname, desc, embedurl) => {
    ...
    try {
      success = await db.run(
        "Update ? SET category = ?, name = ?, description = ?, embedurl = ? WHERE name = ?",
